...
#define LOAD_REDUCTION_FACTOR 0.9   // Define a factor by which the load balancer should reduce theard load
#define LOAD_INCREASE_FACTOR 1.1    // Define factor by which the load balancer should increase thread load
#define CPU_USAGE_TARGET 85         // Define target CPU utilisation to 85%
...
volatile int threadLoad[NUM_THREADS] = {0, 0, 0, 0, 0};
...
const int threadMinLoad[NUM_THREADS] = {0, 10, 10, 10, 0};
const int threadMaxLoad[NUM_THREADS] = {0, 90, 90, 90, 0};
...
void top_load_balance() {
    ...
    for (int tid = 1; tid < NUM_THREADS; tid++) {
        ...
        if (workerLoad > CPU_USAGE_TARGET) {
            // Reduce the load if the thread's utilization is too high
            newLoad = (int)(threadLoad[tid] * (LOAD_REDUCTION_FACTOR));
            SerialUSB.print(" reduce to ");
        } else if (workerLoad < CPU_USAGE_TARGET) {
            // Increase the load if the thread's utilization is too low
            newLoad = (int)(threadLoad[tid] * (LOAD_INCREASE_FACTOR));
        ...
        /*if (sysLoad.threadLoad[tid].loadPerCycle_per < threadMinLoad[tid]) {
            newLoad = threadMinLoad[tid];
            SerialUSB.print(". Normalized to: ");
            SerialUSB.print(newLoad);
        } else if (sysLoad.threadLoad[tid].loadPerCycle_per > threadMaxLoad[tid]) {
            newLoad = threadMaxLoad[tid];
            SerialUSB.print(". Normalized to: ");
            SerialUSB.print(newLoad);
        }*/
        SerialUSB.print("\n");
