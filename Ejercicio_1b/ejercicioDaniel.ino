...
const int threadLoadLowerLimit[NUM_THREADS] = { 0, 1, 1, 1, 0 };
const int threadLoadUpperLimit[NUM_THREADS] = { 0, 1000, 1000, 1000, 0 };
...
*/
    ...
    threadLoad_t * thdLoad = &(sysLoad.threadLoad[4]);
    SerialUSB.println(thdLoad->loadPerCycle_per);
    if (thdLoad->loadPerCycle_per > 15.5){
      SerialUSB.println("CPU mayor de lo que buscamos");
      increaseWorkerLoad(thdLoad->loadPerCycle_per);
    }
  }
}

void increaseWorkerLoad(float cpu_usage_percentage){

  float error = abs(15.0 - cpu_usage_percentage);
  ...
  for (int tid = 1; tid < NUM_THREADS - 1; tid++) {
      // Increase the load if thread load is lower than the upper limit
      if (threadLoad[tid] < threadLoadUpperLimit[tid]){
        threadLoad[tid] = (int)(threadLoad[tid] + control_output);
      }
    }
}
...
/*
    ...
    SerialUSB.print(" ");
    SerialUSB.print(chVTGetSystemTimeX());
*/
